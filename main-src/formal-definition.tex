...
\begin{definition}[Base Reduction]
...
\begin{definition}[Reduction of Term and Type]
...
% A part of algorithmic typing rules
% I removed this paragraph because I wrote in the past.
...
% original rules with the modified application rule. In addition to it, the
% definition of term equivalence is quite different from the original rules. The
% original rules define the term equivalence with the combination of rules.
% However, the algorithmic equivalence between terms are defined by the only one rule: \QAANF.
% \begin{center}
% \infrule[\QAANF]{ \ANF(M) \E_\alpha \ANF(N) }{\G \AV M \E N@A} \hfil
% \end{center}
% This rule calculates the normal form of two given terms under the algorithmic reduction which is defined in Figure \ref{fig:algorithmic-reduction}.
...
Although the original equivalence rules check the equivalence of terms by constructing a derivation tree, the algorithmic equivalence rules calculate the normal form of two terms and compare them with just \( \alpha \)-equivalence.  \( \ANF(M) \) is the normal form of \( M \) under algorithmic reduction which is defined as follows.

\begin{definition}[Algorithmic Base Reduction]
    The relations \( M \longrightarrow_{A\beta} M' \), \( M \longrightarrow_{A\blacklozenge} M' \), and \( M \longrightarrow_{A\Lambda} M' \), and \( M \longrightarrow_{A\%} M' \) are the least compatible relations closed under the rules below.
    \begin{itemize}
        \item \( (\lambda x:\tau.M) N \longrightarrow_{A\beta} M[x \mapsto N] \)
        \item \( \TBL_\alpha (\TB_\alpha M) \longrightarrow_{A\blacklozenge} M \)
        \item \( (\Lambda \alpha.M)\ A \longrightarrow_{A\Lambda} M[\alpha \mapsto A] \)
        \item \( \%_\alpha M \longrightarrow_{A\%} M \qquad (\emptyset = \text{FV}(\%_\alpha M)) \)
    \end{itemize}
\end{definition}

\begin{definition}[Algorithmic Reduction of Term and Type]
    The relations \( M \RA M' \) and \( \tau \RA \tau'
    \) are mutually defined as the least compatible relations closed under the
    rules below.
    \begin{itemize}
        \item If \( M \longrightarrow_{A\beta} M' \), \( M \longrightarrow_{A\blacklozenge} M' \), \( M \longrightarrow_{A\Lambda} M' \) or \( M \longrightarrow_{A\%} M' \) then \( M \longrightarrow M' \). \\
        \item If \( \tau \longrightarrow \tau' \), then \( \lambda x:\tau.M \longrightarrow \lambda:\tau'.M \). \\
        \item If \( M \longrightarrow M' \), then \( \tau\ M \longrightarrow \tau M' \)
    \end{itemize}
\end{definition}

We define the algorithmic reduction using base reduction just the same as the
original reduction. The difference from the original reduction is \( M
\longrightarrow_{A\%} M' \) reduction in Algorithmic Base Reduction. This base
reduction corresponds to \QPercent in the original equivalence rules. In
detail, the side condition \( \emptyset = \FV(\%_\alpha M) \) corresponds to
the premises of \QPercent \( \G\V M:\tau@{A\alpha} \) and \( \G\V M:\tau@A \).

% A part of concrete algorithm
...
functions; {\sc type-inference}, {\sc{kind-inference}}, {\sc
well-formedness-inference}, {\sc{term-equivalence-check}}, {\sc
type-equivalence-check} and {\sc{kind-equivalence-check}}, we show only a part
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \EndIf
        \ElsIf{$M=\lambda x:\tau.N$}
        \If{\Call{type-inference}{$(\G,x:\tau), \Sigma, N, A$} = $\sigma$}
        \State \textbf{return} $\Pi x:\tau.\sigma$
        \Else
        \State \textbf{return} \textbf{Fail}
        \EndIf
        \ElsIf{$M=M'\ N'$}
        \If{\Call{type-inference}{$\G, \Sigma, M', A$} = $\Pi x:\sigma.\tau$ \\
        \hspace{13mm} \textbf{and} \Call{type-inference}{$\G, \Sigma, M', A$} = $\sigma'$ \\
        \hspace{13mm} \textbf{and} \Call{kind-inference}{$\G, \Sigma, \sigma', A$} = $*$ \\
        \hspace{13mm} \textbf{and} \Call{type-equivalence-check}{$\G, \Sigma, \sigma, \sigma', A$} = \textbf{true} }
        \State \textbf{return} \Call{subst}{$\tau, x, N$}
        \Else
        \State \textbf{return} \textbf{Fail}
        \EndIf
        \State \text{// We omit other cases for simplisity.}
        \EndIf
        \EndProcedure
    \end{algorithmic}
\end{algorithm}

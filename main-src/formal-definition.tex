...
algorithmic typing, we can make a type inference algorithm easily just by
...
Unlike all other rules of the algorithmic typing, the term equivalence rules of
algorithmic typing are completely different from the original ones. We replaced
...
compare them with just \( \alpha \)-equivalence. \( \ANF(M) \) is the normal
form of \( M \) under algorithmic reduction as defined in Figure
...
into the surrounding code. See Hanada and Igarashi~\cite{Hanada2014} for
details.

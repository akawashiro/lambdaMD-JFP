...
trees from bottom to up.  Therefore, we need to make all the typing rules
syntax-directed. Using this algorithmic typing rules, we can make a type
inference algorithm easily just by applying the unique appropriate rule one by
one. We show the equivalence ot the original typing rules and the algorithmic
typing rules in Section \ref{sec:properties}.
...
functions; {\sc type-inference}, {\sc{kind-inference}},
{\sc well-formedness-inference}, {\sc{term-equivalence-check}},
{\sc type-equivalence-check} and {\sc{kind-equivalence-check}}, we show only a
part of {\sc type-inference} function because omitted part is very similar to it.

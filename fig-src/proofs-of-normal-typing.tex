...
% \begin{lemma}[Attachment Conversion]
%     \label{lemma:AttachmentConversion}
%     \begin{itemize}
%         \item If \( \G \V \tau \E \tau' :: K @ A \) and \( \G \V K \E K' @ A \) then \( \G \V \tau \E \tau' :: K' @ A \).
%         \item If \( \G \V M \E M' : \tau @ A \) and \( \G \V \tau \E \tau' :: * @ A \) then \( \G \V M \E M' : \tau' @ A \).
%     \end{itemize}
% \end{lemma}
%
% \begin{proof}
%     By induction on the derivation tree of the first judgement.
%     \begin{rneqncase}{\QAbs}{
%             \G\V\lambda x:\tau.M \E \lambda x:\tau'.N : (\Pi x:\tau.\sigma)@A
%             \text{ is derived from } \\
%             \G\V \tau \E \tau' :: *@A \text{ and }
%             \G,x:\tau@A \V M \E N : \sigma @A. \\
%             \text{And } \G \V \Pi x:\tau.\sigma \E \rho :: * @ A.
%         }
%         Using Lemma~\ref{lemma:InversionLemmaForPiType} to \( \G \V \Pi x:\tau.\sigma \E \rho :: * @ A \), there are \( \tau'', \sigma' \) such that \( \G \V \tau \E \tau'' :: * \) and \( \G, x:\tau''@A \V \sigma \E \sigma' :: * \) and \( \rho = \Pi x:\tau''.\sigma' \).
%
%
%         In this case, we can prove using
%         Lemma~\ref{lemma:InversionLemmaForPiType}, the induction hypothesis and
%         \QAbs.
%     \end{rneqncase}
%     \begin{rneqncase}{\QTrans}{
%             \G\V M\E L : \tau@A \text{ is derived from } \\
%             \G\V M\E N : \tau@A \text{ and }
%             \G\V N\E L : \tau@A. \\
%             \text{And } \G \V \tau \E \tau' :: * @ A.
%         }
%         From the induction hypothesis, \( \G\V M\E N : \tau'@A \) and \( \G\V
%         N\E L : \tau'@A \).  Then, we can prove by \QTrans.
%     \end{rneqncase}
%     \begin{rneqncase}{\QRefl}{
%         }
%     \end{rneqncase}
...
\begin{proof}
    ...
    \begin{itemize}
        \item \TAbs \\
            ...
            \begin{itemize}
                \item \( \lambda x:\tau.M \longrightarrow \lambda x:\tau.M' \) \\
                    ...
                    : \sigma @ A \) and get there is \( \sigma' \) such that \(
                    \G \V \sigma \E \sigma' :: * @ A \) and \( \G, x:\tau \V M
                    \E M' : \sigma' @ A \). Now, we can prove with \QAbs.
                \item \( \lambda x:\tau.M \longrightarrow \lambda x:\tau'.M \) \\
        ...
        %             Applying the induction hypothesis to \( \G\V \sigma :: (\Pi
        %             x:\tau.K)@A \), there is \( J \) such that \( \G \V \sigma
        %             \E \sigma' :: J \) and \( \G \V (\Pi x:\tau.K) \E J @ A \). Applying Lemma~\ref{lemma:InversionLemmaForPiType} to \( \G \V (\Pi x:\tau.K) \E J @ A \), 
        %
        %
        %             Now, we can prove using
        %             Lemma~\ref{lemma:InversionLemmaForPiType} and \QTApp.
        %         \item \( \sigma\ M \longrightarrow \sigma M' \) \\
        %             In this case, we can use the induction hypothesis on \(
        %             \G\V M:\tau@A \) and usse \QTApp.
        %     \end{itemize}
        \item \TApp \\
            In this case \( \G \V M\ N : \tau[x\mapsto N]@A \) is derived from \( \G\V M:(\Pi (x:\sigma).\tau)@A \) and \( \G\V N:\sigma@A \). There are two cases for the reduction.
            \begin{itemize}
                \item \( M\ N \longrightarrow M'\ N \) \\
                    Applying the induction hypothesis to \( \G\V M:(\Pi
                    (x:\sigma).\tau)@A \), there is \( \rho \) such that \( \G
                    \V M \E M' : \rho @ A \) and \( \G \V \Pi x:\sigma.\tau \E
                    \rho :: * @ A \). Applying
                    Lemma~\ref{lemma:InversionLemmaForPiType} to \( \G \V \Pi
                    x:\sigma.\tau \E \rho :: * @ A \), there are \( \sigma',
                    \tau' \) such that \( \rho = \Pi x:\sigma'.\tau' \) and \(
                    \G \V \sigma \E \sigma' :: * @ A \) and \( \G, x:\sigma @ A
                    \V \tau \E \tau' :: * @ A \). Applying
                    Lemma~\ref{lemma:Agreement} to \( \G \V M \E M' : \Pi
                    x:\sigma'.\tau' @ A \), \( \G \V M' : \Pi x:\sigma'.\tau' @
                    A \). Because \( \G \V \Pi x:\sigma.\tau \E \Pi
                    x:\sigma'.\tau' :: * @ A \), \( \G \V M' : \Pi
                    x:\sigma.\tau @ A \) from \TConv. Then, \( \G \V M'\ N :
                    \tau[x\mapsto N]@A \) from \TApp.
                \item \( M\ N \longrightarrow M\ N' \) \\
        ...
        \item \TTBL \\
            ...
            \begin{itemize}
                \item \( \TBL_{\alpha}M \longrightarrow \TBL_{\alpha}M' \) \\
                    We can use the induction hypothesis on \( \G\V M:\TW_{\alpha}\tau@A \) and get \( \G \V M \E M' : \TW_{\alpha}\tau'@A \) and \( \G \V \tau \E \tau' :: * @ A \) from Lemma~\ref{lemma:InversionLemmaForTWType}. Then prove using \QTBL.
            ...
            \end{itemize}
        \item \TConv \\
            In this case \( \G\V M:\tau'@A \) is derived from \( \G\V M:\tau@A \) and \( \G\V \tau\equiv \tau':: K@A \). And \( M \longrightarrow M' \). \\
            From the induction hypothesis, \( \G \V M \E M' : \tau'' @ A \) and \( \G \V \tau \E \tau'' :: * @ A \). From \QTRefl, \( \G \V \tau' \E \tau'' :: * @ A \).
    \end{itemize}
\end{proof}

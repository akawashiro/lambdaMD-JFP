        ...
        \item \TApp \\
            ...
            \begin{itemize}
                \item \( M\ N \longrightarrow M'\ N \) \\
                    We can use the induction hypothesis on \( \G\V M:(\Pi (x:\sigma).\tau)@A \) and get \( \G\V M \E M' :(\Pi (x:\sigma).\tau)@A \). From Lemma~\ref{lemma:Agreement}, \( \G \V M' : (\Pi (x:\sigma).\tau)@A \). Then, we can use \TApp for \( M' \) and \( N \).
                \item \( M\ N \longrightarrow M\ N' \) \\
